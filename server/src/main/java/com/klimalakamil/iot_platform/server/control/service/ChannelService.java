                ...
                for (Map.Entry<String, ChannelDeviceInfo> deviceInfoSet : devices.entrySet()) {
                    ...
                    if (channelDeviceInfo.getWorker() != null) {
                        ChannelParticipationRequest request = new ChannelParticipationRequest(
                                channelDeviceInfo.getName(), rootDevice.getName(), channelDeviceInfo.isCanRead(),
                                channelDeviceInfo.isCanWrite());

                        expectedMessages.add(expectedMessage.sendAndExpect(request, channelDeviceInfo.getWorker(),
                                new OnDeviceStateMessageListener(devices, name), 15, TimeUnit.SECONDS));
                    } else {
